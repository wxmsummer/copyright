...
func (this *RegisterController) Post() {
	...
	// 用户注册的时候，生成公钥和私钥，并返回私钥给用户
	privateKey, publicKey, err := utils.GenerateKeys()
	if err != nil {
		log.Fatal(err)
	}
	fmt.Println("publicKey:",publicKey)
	user := models.User{0, username, password, publicKey,0, time.Now().Unix()}
	_, err = models.InsertUser(user)
	if err != nil {
		this.Data["json"] = map[string]interface{}{"code": 0, "message": "注册失败"}
	} else {
		this.Data["json"] = map[string]interface{}{"code": 1, "message": "注册成功", "privateKey":privateKey}
	}
	this.ServeJSON()
